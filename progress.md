> **2025-10-11 注記**: 2025-10-08 以前のログは `nexus-actor-*` 系クレート名で記録されています。現在は `cellex-actor-*` へリネーム済みのため、同名コマンドは適宜読み替えてください。
...
2025-10-26 進捗: queue-v2 構成で TestMailboxFactory が `QueueRwCompat` を直接利用するよう更新し、QueueMailbox/QueueMailboxProducer/QueueMailboxRecv を `QueueMailboxInternal` 経由の委譲構造へ統一。メトリクス通知と ready 通知の責務を内包化した上で `cargo test -p cellex-actor-core-rs --tests` と queue-v1 fallback の両テストを実施し成功を確認。
2025-10-26 進捗: queue-v1 退役フェーズ完了。workspace 全体から `queue-v2` フィーチャーフラグを撤廃し、`SyncQueueDriver` を常時有効化。Cargo 設定とテスト群を無条件で v2 構成へ統一し、`./scripts/ci-check.sh all` を前提にした新しい運用方針を docs/tasks へ記録。
