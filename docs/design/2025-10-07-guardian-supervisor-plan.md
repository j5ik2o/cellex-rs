...
- 旧 cellex-rs (docs/sources/cellex-rs/modules/actor-std/src/actor/guardian.rs 等)
...
- typed DSL の第一段階として `Props<U, R>` / `ActorSystem<U, R>` / `ActorRef<U, R>` を導入済み。
  - `Props::new` / `Props::with_behavior` が `MessageEnvelope<U>` を生成し、ユーザーメッセージを型安全に処理する。
  - `ActorSystem::<U, R>::new(runtime)` と `RootContext::spawn` で typed アクターを起動できる。
  - `ActorRef::<U, R>::tell` はユーザーメッセージを型安全に送信し、SystemMessage は `MapSystemShared` を介して優先度付きチャネルで処理される。
