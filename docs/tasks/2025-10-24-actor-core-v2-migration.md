...
- **QueueMailbox のコア差し替え本体**  
  ...
  - embedded 系では `ArcMailboxFactory` / `LocalMailboxFactory` を `SyncQueueDriver` 利用へ切り替え、`queue-v1` フィーチャ時のみレガシードライバを保持。priority mailbox 系と `QueueRwCompat` を利用する経路の整理が残タスク。
...
- **全体テスト・CI とドキュメント更新**  
  ...
  - [x] `QueueError` → `MailboxError` 変換テーブルを実装し、単体テストで網羅性と整合性を検証する。
    ...
    - 2025-10-25: queue-v2 ルートで `TestMailboxFactory` および Tokio mailbox 系のファクトリ／Sender／Mailbox 実装を `SyncQueueDriver` ベースへ移行。queue-v1 ビルドのみ `LegacyQueueDriver<QueueRwCompat<_>>` を利用し続ける構成に整理し、Backpressure/メトリクス処理を新ドライバへ委譲。
    - 2025-10-25: `./scripts/ci-check.sh all` を実行し、`SyncQueueDriver` 経路（queue-v2 既定）および queue-v1 フォールバックの双方でテスト／lint が完走することを確認。Tokio mailbox の回帰テスト（`cargo test -p cellex-actor-std-rs tokio_mailbox`）も追加で実行済み。
