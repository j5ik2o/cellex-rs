...
fn bench_duplicate_detection(c: &mut Criterion) {
  ...
  for dup_ratio in [0, 25, 50, 75].iter() {
    group.bench_with_input(BenchmarkId::new("duplicate_ratio", dup_ratio), dup_ratio, |b, &dup_ratio| {
      b.iter(|| {
        let mut coordinator = DefaultReadyQueueCoordinator::new(32);
        let total = 10_000;
  ...
  for num_threads in [1, 2, 4, 8].iter() {
    group.bench_with_input(BenchmarkId::new("threads", num_threads), num_threads, |b, &num_threads| {
      b.iter(|| {
          ...
          .map(|thread_id| {
            ...
            thread::spawn(move || {
              for i in 0..items_per_thread {
                ...
                coordinator_clone.lock().unwrap().register_ready(MailboxIndex::new(idx, 0));
              }
            })
          })
          .collect();
        ...
        coordinator.lock().unwrap().drain_ready_cycle(total_expected, &mut out);
...
fn bench_concurrent_comparison(c: &mut Criterion) {
  ...
  for num_threads in [1, 2, 4, 8].iter() {
    ...
    group.bench_with_input(BenchmarkId::new("default_locked", num_threads), num_threads, |b, &num_threads| {
      b.iter(|| {
        let coordinator = Arc::new(Mutex::new(DefaultReadyQueueCoordinator::new(32)));
        let items_per_thread = 10_000usize;
    ...
    group.bench_with_input(BenchmarkId::new("lockfree", num_threads), num_threads, |b, &num_threads| {
      b.iter(|| {
...
fn bench_v1_vs_v2_comparison(c: &mut Criterion) {
  ...
  for num_threads in [1, 2, 4, 8].iter() {
    ...
    group.bench_with_input(BenchmarkId::new("v1_with_mutex", num_threads), num_threads, |b, &num_threads| {
      b.iter(|| {
        let coordinator = Arc::new(Mutex::new(LockFreeCoordinator::new(32)));
        let items_per_thread = 10_000usize;
    ...
    group.bench_with_input(BenchmarkId::new("v2_no_mutex", num_threads), num_threads, |b, &num_threads| {
      b.iter(|| {
