...
fn escalation_stage_increments_with_parent_hops() {
  ...
  assert!(matches!(failure.failure_escalation_stage, FailureEscalationStage::Initial));

  let parent_failure = failure.escalate_to_parent().expect("parent exists");
  assert!(matches!(parent_failure.failure_escalation_stage, FailureEscalationStage::Escalated { hops: 1 }));

  let root_failure = parent_failure.escalate_to_parent().expect("root exists");
  assert!(matches!(root_failure.failure_escalation_stage, FailureEscalationStage::Escalated { hops: 2 }));
